...
struct real_roots_internal {
    static real_roots_result<typename Poly::template RootType<Number>> real_roots(const Poly& polynomial, const Interval<Number>& interval) {
        CARL_LOG_FATAL("carl.ran", "real_roots not implemented for type: " << typeid(Poly).name() << " " << polynomial << " " << interval);
        assert(false);
        return real_roots_result<typename Poly::template RootType<Number>>::no_roots_response();
    }

    static real_roots_result<typename Poly::template RootType<Number>> real_roots(const Poly& polynomial, const std::map<Variable, typename Poly::template RootType<Number>>& assignment, const Interval<Number>& interval) {
...
struct real_roots_internal<UnivariatePolynomial<Coeff>, Number> {
    static real_roots_result<typename UnivariatePolynomial<Coeff>::template RootType<Number>> real_roots(const UnivariatePolynomial<Coeff>& polynomial, const Interval<Number>& interval) {
    ...
    static real_roots_result<typename UnivariatePolynomial<Coeff>::template RootType<Number>> real_roots(const UnivariatePolynomial<Coeff>& polynomial, const std::map<Variable, typename UnivariatePolynomial<Coeff>::template RootType<Number>>& assignment, const Interval<Number>& interval) {
...
real_roots_result<typename Poly::template RootType<Number>> real_roots(const Poly& polynomial, const std::map<Variable, typename Poly::template RootType<Number>>& assignment, const Interval<Number>& interval = Interval<Number>::unbounded_interval()) {
    return internal::real_roots_internal<Poly, Number>::real_roots(polynomial, assignment, interval);
}

template<typename Poly, typename Number = typename UnderlyingNumberType<typename Poly::CoeffType>::type, typename... Coeff>
real_roots_result<typename Poly::template RootType<Number>> real_roots(const Poly& polynomial, const Interval<Number>& interval = Interval<Number>::unbounded_interval()) {
