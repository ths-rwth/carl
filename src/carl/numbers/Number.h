...
//      auch bei Sachen wie gcd, soll das innerhalb oder außerhalb der Klasse sein?
	...
	public:
		...
		BaseNumber(const std::string& s) : mData(s) {} //TODO: this should be done with rationalize probably
...
//***************************************************************   CLN:
	...
	class Number<cln::cl_RA> : public BaseNumber<cln::cl_RA> {
		...
		//could probably changed such that it's the same as in all other classes
		inline bool isZero() {
			return zerop(mData);
		}
		...
		//TODO: probably substitute rationalize with constructor
		inline Number<cln::cl_RA> log() {
			return cln::rationalize(cln::realpart(cln::log(mData)));
		}

		inline Number<cln::cl_RA> sin() {
			return cln::rationalize(cln::sin(mData));
		}

		inline Number<cln::cl_RA> cos() {
			return cln::rationalize(cln::cos(mData));
		}
