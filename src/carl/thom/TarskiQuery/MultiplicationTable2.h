...
public:
        ...
        MultiplicationTable2() : mTable(), mBase(), mGb() {}
        ...
        template<typename C>
        friend std::ostream& operator<<(std::ostream& o, const MultiplicationTable2<C>& table);
        ...
        void init(const GroebnerBase<Number>& gb) {
                ...
                for(const auto& m : Bor) {
                        if(std::find(Cor.begin(), Cor.end(), m) != Cor.end()) {
                                CARL_LOG_TRACE("carl.thom.tarski", "in bor and also in cor: " << m);
                                ...
                                CARL_LOG_TRACE("carl.thom.tarski", "not in cor but in bor: " << m);
                                auto it = vars.begin();
                                Variable var = *it;
                                Monomial x_beta;
...
template<typename C>
std::ostream& operator<<(std::ostream& o, const MultiplicationTable2<C>& table) {
