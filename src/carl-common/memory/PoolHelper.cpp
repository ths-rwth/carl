...
std::size_t RehashPolicy::numBucketsFor(std::size_t numElements) const {
	const float minBuckets = float(numElements) / mMaxLoadFactor;
	const unsigned long* p = std::lower_bound(primes, primes + numPrimes, minBuckets);
	mNextResize = static_cast<std::size_t>(std::ceil(float(*p) * mMaxLoadFactor));
	return *p;
}

std::pair<bool, std::size_t> RehashPolicy::needRehash(std::size_t numBuckets, std::size_t numElements) const {
	if (numElements > mNextResize) {
		float minBuckets = float(numElements) / mMaxLoadFactor;
		if (minBuckets > float(numBuckets)) {
			minBuckets = std::max(minBuckets, mGrowthFactor * float(numBuckets));
			const unsigned long* p = std::lower_bound(primes, primes + numPrimes, minBuckets);
			mNextResize = static_cast<std::size_t>(std::ceil(float(*p) * mMaxLoadFactor));
			return std::make_pair(true, *p);
		} else {
			mNextResize = static_cast<std::size_t>(std::ceil(float(numBuckets) * mMaxLoadFactor));
...
const unsigned long RehashPolicy::primes[256 + 48 + 1] = {
