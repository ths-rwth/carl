...
bool represents(const carl::RealAlgebraicNumberInterval<Number> root, const Number& exact) {
	if (root.is_numeric()) {
...
TEST(RootFinder, real_roots)
{
...

	{
		...
		std::map<carl::Variable, carl::RealAlgebraicNumberInterval<Rational>> m;
		m.emplace(y, carl::RealAlgebraicNumberInterval<Rational>(Rational(-1)));
...
TEST(RootFinder, evalRoots)
{
	...
	carl::RealAlgebraicNumberInterval<Rational> xval(xpoly, xint);
...
TEST(RootFinder, Chebyshev)
{
	...
	carl::RealAlgebraicNumberInterval<Rational> mone(Rational(-1));
	carl::RealAlgebraicNumberInterval<Rational> pone(Rational(1));
